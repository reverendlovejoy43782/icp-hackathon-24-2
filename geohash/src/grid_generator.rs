...
pub fn create_grid_with_geohash(lat_start: f64, lon_start: f64, lat_end: f64, lon_end: f64, step_meters: f64) -> Vec<(f64, f64, String, Area)> {
    ...
    // Print the details of the area
    println!("AREA BOUNDS: LAT_START = {}, LON_START = {}, LAT_END = {}, LON_END = {}", lat_start, lon_start, lat_end, lon_end);
    println!("STEP SIZE IN DEGREES: LAT_STEP = {}", step_degrees_lat);
    ...
    for &lat in &latitudes {
        ...
        println!("LATITUDE: {}, STEP SIZE IN DEGREES FOR LONGITUDE: {}", lat, step_degrees_lon);
        ...
        for &lon in &longitudes {
            ...
            println!(
                "SQUARE CENTER: ({}, {}), GEOHASH: {}, BOUNDS: {:?}, DIMENSIONS: ({}m x {}m)",
                center_lat, center_lon, geohash_key, square_bounds, step_meters, step_meters
            );
...
pub fn create_grid_with_geohash(lat_start: f64, lon_start: f64, lat_end: f64, lon_end: f64, step_meters: f64) -> Vec<(f64, f64, String, Area)> {
    ...
    while lat < lat_end {
        ...
        let meters_in_degree_lon = METERS_IN_DEGREE_LAT * (lat.to_radians().cos());
        // Calculate step size in degrees for longitude
        let step_degrees_lon = step_meters / meters_in_degree_lon;

        // Populate longitudes within the specified range
        let mut lon = lon_start;
        while lon < lon_end {
            // Calculate the center point of each grid square
            let center_lat = lat + step_degrees_lat / 2.0;
            ...
            // Calculate bounds for the square
            let square_bounds = Area {
                lat_start: lat,
                lon_start: lon,
                lat_end: lat + step_degrees_lat,
                lon_end: lon + step_degrees_lon,
            };
...
// New function to create a grid with geohashes
pub fn create_grid_with_geohash(lat_start: f64, lon_start: f64, lat_end: f64, lon_end: f64, step_meters: f64) -> Vec<(f64, f64, String, Area)> {
    // Conversion factor from meters to degrees latitude
    const METERS_IN_DEGREE_LAT: f64 = 111320.0;
    ...
    while lat <= lat_end {
        ...
        while lon <= lon_end {
            ...
            // Calculate bounds for the square
            let square_bounds = Area {
                lat_start: center_lat - step_degrees_lat / 2.0,
                lon_start: center_lon - step_degrees_lon / 2.0,
                lat_end: center_lat + step_degrees_lat / 2.0,
                lon_end: center_lon + step_degrees_lon / 2.0,
            };
