...
fn compute_geohash(geolocation: Geolocation) -> (Area, String) {
    ...
    ic_cdk::println!("Computed geohash: {}", nearest_geohash);
    ic_cdk::println!("Bounds: {:?}", bounds);

    // Return the area and the nearest geohash
    (area, nearest_geohash)
}
...
fn query_compute_area(geohash: String) -> AreaResponse {
    // Decode the geohash back into coordinates
    let coord = match decode_geohash(&geohash) {
        Ok(c) => c,
