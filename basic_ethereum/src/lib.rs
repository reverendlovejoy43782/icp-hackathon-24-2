...
#[update]
pub async fn ethereum_address(owner: Option<Principal>, geohash: String) -> String {
    let caller = validate_caller_not_anonymous();
    let owner = owner.unwrap_or(caller);
    let wallet = EthereumWallet::new(owner, &geohash).await;
    // You can use geohash here if needed
...
#[update]
pub async fn transaction_count(owner: Option<Principal>, block: Option<BlockTag>, geohash: String) -> Nat {
    let caller = validate_caller_not_anonymous();
    let owner = owner.unwrap_or(caller);
    let wallet = EthereumWallet::new(owner, &geohash).await;
...
#[update]
pub async fn send_eth(to: String, amount: Nat, geohash: String) -> String {
    ...
    let nonce = nat_to_u64(transaction_count(Some(caller), Some(BlockTag::Latest), geohash.clone()).await);
    ...
    let wallet = EthereumWallet::new(caller, &geohash).await;
    let tx_hash = transaction.signature_hash().0;
    let (raw_signature, recovery_id) = wallet.sign_with_ecdsa(geohash.clone(), tx_hash).await;
