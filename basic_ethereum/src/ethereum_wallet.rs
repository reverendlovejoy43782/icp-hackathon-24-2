...
impl EthereumWallet {
    pub async fn new(owner: Principal, geohash: &str) -> Self {
        let derived_public_key = derive_public_key(&owner, &lazy_call_ecdsa_public_key().await, &geohash);
        Self {
            owner,
            derived_public_key,
        }
    ...
    pub async fn sign_with_ecdsa(&self, geohash: String, message_hash: [u8; 32]) -> ([u8; 64], RecoveryId) {
        use ic_cdk::api::management_canister::ecdsa::SignWithEcdsaArgument;

        let derivation_path = derivation_path(&self.owner, &geohash);
...
fn derive_public_key(owner: &Principal, public_key: &EcdsaPublicKey, geohash: &str) -> EcdsaPublicKey {
    use ic_crypto_extended_bip32::{DerivationIndex, DerivationPath};
    let derivation_path = DerivationPath::new(
        derivation_path(owner, geohash)
            .into_iter()
            .map(DerivationIndex)
            .collect(),
    );
...
fn derivation_path(owner: &Principal, geohash: &str) -> Vec<Vec<u8>> {
    const SCHEMA_V1: u8 = 1;
    [
        ByteBuf::from(vec![SCHEMA_V1]),
        ByteBuf::from(owner.as_slice().to_vec()),
        ByteBuf::from(geohash.as_bytes().to_vec()),
    ]
    .iter()
    .map(|x| x.to_vec())
    .collect()
